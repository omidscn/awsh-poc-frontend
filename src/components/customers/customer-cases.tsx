...
export function CustomerCases({ cases }: { cases: CaseWithAgent[] }) {
  return (
    <Card>
      ...
      <CardContent>
        {cases.length === 0 ? (
          <p className="text-sm text-surface-500">Keine Fälle vorhanden.</p>
        ) : (
          <div className="space-y-3">
            {cases.map((c) => (
              <Link
                key={c.id}
                href={`/cases/${c.id}`}
                className="block rounded-md border border-surface-700/50 p-3 transition-all duration-200 hover:bg-surface-800/50 hover:translate-y-[-1px] hover:shadow-lg hover:shadow-black/10"
              >
