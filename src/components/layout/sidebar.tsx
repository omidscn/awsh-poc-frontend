...
export function Sidebar() {
  ...
  useEffect(() => {
    ...
    const offset = document.querySelector(".sidebar-offset") as HTMLElement;
    if (offset) offset.style.marginLeft = width;
  }, [collapsed]);
  ...
  return (
    <aside
      className={cn(
        "fixed inset-y-0 left-0 z-50 flex flex-col border-r border-surface-700 bg-surface-900 transition-all duration-300 ease-in-out",
        collapsed ? "w-16" : "w-60"
      )}
    >
      {/* Logo */}
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active
                  ? "bg-surface-700 border-[#34d399] text-[#34d399]"
                  : "border-transparent text-surface-500 hover:bg-surface-800 hover:text-surface-200"
              )}
            >
              <span className={cn("shrink-0", active ? "text-[#34d399]" : "")}>
                {item.icon}
              </span>
      ...
      <div className="border-t border-surface-700 p-2">
        <button
          ...
          className="flex w-full items-center justify-center rounded-lg p-2.5 text-surface-500 transition-colors hover:bg-surface-800 hover:text-surface-200"
